...
此页面假定你基本了解代码优先。 请查看下面的演练的第一个代码的详细信息：

-   [代码优先的新数据库](~/ef6/modeling/code-first/workflows/new-database.md)
-   [代码优先的现有数据库](~/ef6/modeling/code-first/workflows/existing-database.md)
...
在此步骤中将定义 VB.NET POCO 实体类型表示概念模型。 类不需要任何在基类中派生或实现任何接口。

-   将新类添加到项目中，输入**SchoolModel**的类名
-   新类的内容替换为以下代码
...
## <a name="define-a-derived-context"></a>定义派生的上下文

我们想要开始使用从实体框架的类型，因此我们需要添加 EntityFramework NuGet 包。
...
-   将新类添加到项目中，输入**SchoolContext**的类名
-   新类的内容替换为以下代码
...
## <a name="configuring-with-the-fluent-api"></a>使用 Fluent API 配置

本部分演示如何使用 fluent Api 来配置对映射到列映射和表之间的关系的属性的表类型\\在模型中的类型。 通过 fluent API 公开**DbModelBuilder**键入和最常访问通过重写**OnModelCreating**方法**DbContext**。

-   将以下代码复制并将其添加到**OnModelCreating**上定义的方法**SchoolContext**类注释说明每个映射的作用
...
让我们来执行某些数据访问使用**SchoolContext**若要查看操作中的模型。

-   打开 Module1.vb 文件，其中定义 Main 函数
-   复制并粘贴以下 Module1 定义
