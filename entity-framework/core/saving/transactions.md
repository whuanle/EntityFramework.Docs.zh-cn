...
默认情况下，如果数据库提供程序支持事务，则会对 `SaveChanges()` 的单一调用中的所有更改都将应用到事务中。 如果所有更改均失败，则会回滚事务且所有更改都不会应用到数据库。 这意味着，`SaveChanges()` 可保证要么完全成功，要么在出现错误时不修改数据库。

对于大多数应用程序，此默认行为已足够。 只有在应用程序需求认为有必要时，才应该手动控制事务。

## <a name="controlling-transactions"></a>控制事务

可以使用 `DbContext.Database` API 开始、提交和回滚事务。 以下示例显示了在单个事务中正执行的两个 `SaveChanges()` 操作以及 LINQ 查询。
